...
def replace_char(frags):
    for line in frags:
        line[0] = "{}{}".format("X",element(line[0][0]).atomic_number)
    return frags

def build_basis(elements,basis_set):
    basis = {}
    for ele in elements:
        basis[ele] = basis_set
        ghost_atm = "{}{}".format("X",element(ele).atomic_number)
        basis[ghost_atm] = gto.basis.load(basis_set,ele)
    return basis
