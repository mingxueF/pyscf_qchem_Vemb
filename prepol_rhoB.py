...
def prepol_B(frags, basis,charges):
    mola = gto.M(
        atom=frags["A"],
        basis=basis,
        charge=-1,
    ...
    mf = scf.RHF(mola).run()          # ground-state HF wave-function
    
    # --- CHELPG fit (PySCF ≥2.3 has the ESP-fitting helpers) ---
    #xyz_grid, esp = tools.esp.get_esp(mola, mf.make_rdm1(), grid_spacing=0.25)
    #chelpg = tools.esp.fit_charges(
    #    mola, xyz_grid, esp, method="CHELPG"
    #)                                 # returns per-atom charges
    
    #q_S, q_C, q_N = chelpg           # save these three numbers
    #coor_S, coor_C, coor_N = mola.atom_coords(unit="Angstrom")
    #
    #
    ...
    coords = mola.atom_coords(unit="Angstrom")  # shape (3,3)
    ...
    mf = scf.RHF(molb)                              # or scf.ROHF/DFT…
